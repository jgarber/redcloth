...
VALUE
redcloth_clean(machine, self, p, pe)
  int machine;
  VALUE self;
  char *p, *pe;
{
...
VALUE
redcloth_sanitize(self, str)
  VALUE self, str;
{
  StringValue(str);
  int cs = redcloth_attributes_en_inline;
  return redcloth_clean(cs, self, RSTRING_PTR(str), RSTRING_PTR(str) + RSTRING_LEN(str) + 1);
}

VALUE
redcloth_filter(self, str)
  VALUE self, str;
{
  StringValue(str);
